...
import colorsys
import time
from Utils.ultrasoundPreprocessingFunctions import compute_phase_symmetry
...
def objective_function(params,pcd_US,pcd_CT,intensity_a,threshold,initial_translation,initial_euler):
    ...
    distance_optimized=compute_fiducials_deviation(initial_translation, initial_euler, translation, rotation_params)
...
def process_rows(args):
    ...
    ct_file_folder = os.path.join(os.path.dirname(dataFolder), 'CTs')
    ct_file_paths = []
    # pcd_CT=o3d.geometry.PointCloud()
    ...
    sweep_df = pd.read_csv(os.path.join(dataFolder, 'sweepProcessed.csv'))
    ...
    for i, row in sweep_df.iterrows():
        ...
        #     continue
        start = time.time()
        img_path = os.path.join(img_folder, row['file_path'])
        ...
        if os.path.isfile(img_path):
            ...
            img_enhanced, pcd_CT = enhanceContrast(img, ct_file_paths,
                                                   calibration_t, calibration_euler, scale_X, scale_Y, T_tracking,
                                                   intcDisThreshold)
            ...
            pcd_US_selected_initial_registered = copy.deepcopy(pcd_US_selected).transform(T_tracking)
            ...
            if visualization_flag:
                ...
                o3d.visualization.draw_geometries(
                    [copy.deepcopy(pcd_US_selected).transform(T_tracking), pcd_CT_selected])
...
if __name__=='__main__':
    dataFolder="Z:/AI_Ultrasound_dataset/cadaver02_F231218/Linear18/record01"
    parallel_process_dataframe(dataFolder, num_processes=8)
